...
## Canonical target inesistente: `Themes/docs/shared-components/` (finding 2026-09-17)

Verificato in questo audit: **`Themes/docs/` non esiste nel repository** (solo
`Themes/<Tema>/` esistono, nessuno ha una cartella `docs/shared-components/`).
Nonostante questo, circa 30 file `.md`/`.txt` sotto `Modules/Gdpr/docs/` sono
stati sovrascritti (già nel commit iniziale `1d66d618`, non da una sessione
recente — vedi `git log --follow` su uno qualsiasi di questi file) con un
identico stub a 8 righe (front-matter `module: theme` / `canonical:
../../.../Themes/docs/shared-components/<nome>.md` + una riga "See canonical
documentation: ..."), il cui target non esiste. Colpisce anche ~24 placeholder
`.gitkeep` sotto `raw/**` e `wiki/**`, che dovrebbero restare vuoti (0 byte) e
invece contengono questo stesso stub — innocuo per git ma fuorviante per chi
li apre. Alcuni nomi target sono anche corrotti da una concatenazione
difettosa (`.gitkeep-Modules`, `README-Modules.md`,
`2025-q4-roadmap-Modules.md`).

File toccati in questo audit per essere effettivamente letti (`architecture/structure.md`,
riscritto con contenuto reale) — gli altri ~29 sono stati lasciati intatti
per non trasformare questo passaggio in una riscrittura totale. Riprodurre con:

```bash
cd Modules/Gdpr/docs && grep -rl "shared-components" .
```

Non è chiaro se `Themes/docs/shared-components/` fosse un piano di
consolidamento mai eseguito o un artefatto di un tool di scaffolding
difettoso: prima di scrivere altro contenuto in questi file va deciso quale
delle due cose è, altrimenti si rischia di inventare documentazione per un
target che dovrebbe invece essere creato altrove (nel modulo `Themes`) o i cui
file andrebbero invece scritti direttamente qui con contenuto reale.
